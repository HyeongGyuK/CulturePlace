...
<head>
	...
	<script type="text/javascript">
		function checkForm(){
			var b_title = document.join.b_title.value;
			if(b_title == ""){
				alert('제목을 입력해주세요.');
				document.join.b_title.focus();
				return false;
			}
			
			var b_category = document.join.b_category.value;
			if(b_category == ""){
				alert('카테고리를 선택해주세요.');
				document.join.b_category.focus();
				return false;
			}
		...
		<div class="col-sm-offset-<%=offset %> col-sm-<%=content%>">
			<div class="panel">
				...
				<div class="panel-body" class="col-sm-<%=twelve%>">
					<form name= "join" class="form-horizontal" action="/board/review_insert" method="post" enctype="multipart/form-data">
					<input type="hidden" name="command" value="boardReview">
						...
						<div class="form-group" >
							<div class="col-sm-<%=twelve %>">
								<textarea class="form-control" rows="10" id="b_content" name="board_content" 
						...
						<div class="btn-group btn-group-justified">
							<div class="btn-group">
								<button type="submit" id="submit" name="submit"
									class="btn btn-default" onclick="return checkForm();">등록하기</button>
							</div>
						</div>
					</form>
				</div>
